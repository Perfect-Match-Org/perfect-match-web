...
export default function HybridTemplateEditor({ template, onSave, onCancel }: TemplateEditorProps) {
			...
			switch (block.type) {
				case "logo":
					...
					if (block.content.subtitle) {
						htmlParts.push(
							`                <p style="font-size: 14px; color: #666; margin: 5px 0 0 0;">${block.content.subtitle}</p>`,
						);
					}
					htmlParts.push("            </div>");
					break;
				...
				case "heading":
					const headingStyles = Object.entries(block.styles)
				...
				case "text":
					const textStyles = Object.entries(block.styles)
						.map(([key, value]) => `${key.replace(/([A-Z])/g, "-$1").toLowerCase()}: ${value}`)
						.join("; ");
				...
				case "button":
					const buttonStyles = Object.entries(block.styles)
						.map(([key, value]) => `${key.replace(/([A-Z])/g, "-$1").toLowerCase()}: ${value}`)
						.join("; ");
					htmlParts.push(`            <div style="text-align: center;">`);
					htmlParts.push(
						`                <a href="${block.content.link}" class="cta-button" style="${buttonStyles}">${block.content.text}</a>`,
					);
					htmlParts.push("            </div>");
					break;
	...
	const getDefaultContent = (blockType: string) => {
		switch (blockType) {
			...
			case "text":
				return { text: "Your text content goes here. You can add personalization tokens like {{firstName}}." };
			case "button":
				return { text: "Click Here", link: "{{ctaLink}}" };
			case "image":
				return { src: "https://via.placeholder.com/400x200", alt: "Image description" };
			case "logo":
				return { text: "Perfect Match", subtitle: "Find Your Perfect Match" };
			case "spacer":
				return { height: "20px" };
			default:
				return {};
	...
	const getDefaultStyles = (blockType: string) => {
		switch (blockType) {
			case "heading":
				return { fontSize: "24px", color: "#161616", marginBottom: "15px", fontWeight: "bold" };
			case "text":
				return { fontSize: "16px", color: "#161616", lineHeight: "1.6", marginBottom: "15px" };
			case "button":
				return { backgroundColor: "#FF328F", color: "#ffffff", padding: "12px 24px", borderRadius: "6px", textAlign: "center" };
			case "image":
				return { maxWidth: "100%", height: "auto", marginBottom: "15px" };
			case "logo":
				return { textAlign: "center", marginBottom: "20px" };
			case "divider":
				return { height: "1px", backgroundColor: "#e5e5e5", margin: "20px 0" };
			case "spacer":
				return { height: "20px" };
			default:
				return {};
			...
			<div className="flex-1 p-4">
				<div className="bg-white rounded-lg border min-h-96 p-6" style={{ maxWidth: "600px", margin: "0 auto" }}>
					{blocks.map((block, index) => (
						<div key={block.id} className="relative group mb-4 p-2 border border-transparent hover:border-gray-300 rounded">
							{/* Block Controls */}
			...
			case "logo":
				return (
					<div style={{ textAlign: block.styles.textAlign }}>
						...
						{block.content.subtitle && <p className="text-sm text-gray-600 mt-1">{block.content.subtitle}</p>}
					</div>
				);
