...
export default function EmailMarketing() {
    ...
    const handleSaveCampaign = useCallback(async (campaign: Campaign, action: string) => {
        try {
            ...
            const url = campaign._id ? `/api/admin/email/campaigns/${campaign._id}/${["send", "schedule"].includes(action) ? "send" : ""}` : "/api/admin/email/campaigns";
            const response = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(campaign) });
            if (response.ok) {
