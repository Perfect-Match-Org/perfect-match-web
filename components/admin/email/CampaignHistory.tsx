// components/admin/email/CampaignHistory.tsx
...
export default function CampaignHistory({ onEditCampaign, onViewAnalytics }: CampaignHistoryProps) {
  ...
  const fetchCampaigns = async () => {
    try {
      ...
      const response = await fetch(`/admin/email/campaigns?${params}`);
  ...
  const handleAction = async (campaignId: string, action: string) => {
    try {
      ...
      switch (action) {
        case 'send':
          endpoint = `/admin/email/campaigns/${campaignId}/send`;
          break;
        case 'pause':
          endpoint = `/admin/email/campaigns/${campaignId}/pause`;
          break;
        case 'resume':
          endpoint = `/admin/email/campaigns/${campaignId}/resume`;
          break;
        case 'delete':
          endpoint = `/admin/email/campaigns/${campaignId}`;
          method = 'DELETE';
          break;
      }
                      ...
                      {campaign.analytics.delivered_count > 0 ? (
                        <div className="text-sm">
                          <div className="flex items-center space-x-4">
                            <div>
                              <span className="font-medium text-gray-900">{calculateOpenRate(campaign)}%</span>
                              <span className="text-xs text-gray-500 ml-1">open</span>
                            </div>
                            <div>
                              <span className="font-medium text-gray-900">{calculateClickRate(campaign)}%</span>
