				...
				<div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-8 flex flex-col md:flex-row gap-4">
					...
					<div className="flex-1 overflow-x-auto pb-2 md:pb-0">
						<div className="flex gap-2 min-w-max">
							<div className="flex items-center text-gray-500 py-2 text-sm mr-2 font-medium">
							...
							{allTags.map((tag) => (
								<button
									...
									className={`px-3 rounded-md items-center text-xs font-medium transition-colors ${
											...
											{(template.year || (template.tags && template.tags.length > 0)) && (
												<div className="flex flex-wrap gap-1.5 mb-4">
													...
													{template.tags?.slice(0, 3).map((tag, idx) => (
														<span
															key={idx}
															className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-[10px] font-medium uppercase tracking-wider"
														>
															{tag}
														</span>
													))}
