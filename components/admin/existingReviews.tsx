		...
		<div className="space-y-6">
			...
			{totalPages > 1 && (
				<div className="flex items-center justify-center space-x-6 rounded-xl border border-gray-200 bg-white px-6 py-3 shadow-sm">
					<button
						className={`rounded-md bg-gray-50 px-5 py-2 text-xs font-bold uppercase tracking-widest text-gray-700 transition-all ${
							currentPage === 1 || !!actionLoading ? "cursor-not-allowed opacity-30" : "active:scale-95 hover:bg-gray-100/80"
						}`}
					...
					<button
						className={`rounded-md bg-gray-50 px-5 py-2 text-xs font-bold uppercase tracking-widest text-gray-700 transition-all ${
							currentPage === totalPages || !!actionLoading
								...
								: "active:scale-95 hover:bg-gray-100/80"
						}`}
						onClick={() => onPageChange(currentPage + 1)}
			...
			<div className="relative min-h-[320px]">
				...
				<div
					className={`space-y-6 transition-opacity duration-300 ${loading && existingReviews.length === 0 ? "opacity-0" : "opacity-100"}`}
				>
