...
import { LayoutDashboard, FileCode, MessageSquare, Mail, ChevronLeft, ChevronRight, LogOut, User, Bell, Search } from "lucide-react";
						...
						return (
							<button
								...
								className={`w-full flex items-center px-3 py-2.5 rounded-xl transition-all duration-200 group ${
									isActive ? "bg-pink-50 text-pink-600 shadow-sm" : "text-gray-500 hover:bg-gray-50 hover:text-gray-900"
								}`}
							>
								<Icon
								...
								/>
								{!isSidebarCollapsed && <span className="ml-3 font-semibold text-sm">{item.name}</span>}
								{!isSidebarCollapsed && isActive && <div className="ml-auto w-1.5 h-1.5 bg-pink-600 rounded-md" />}
							</button>
						);
					})}
				</nav>
					...
					<div className="flex items-center gap-4">
						...
						<div className="flex items-center gap-3">
							<div className="text-right hidden sm:block">
								<p className="text-xs font-bold text-gray-900 leading-tight">{session?.user?.name ?? "Admin User"}</p>
								<p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest leading-tight">Super Admin</p>
							</div>
