...
function Review({
    ...
    status,
    ...props
}: Omit<React.ComponentPropsWithoutRef<"figure">, keyof Review> &
    Review & { createdAt?: string; updatedAt?: string; _id?: string; __v?: number; status?: string; id?: string }) {
...
function splitArray<T>(array: Array<T>, numParts: number) {
    // Handle edge cases
    let result: Array<Array<T>> = Array(numParts)
        .fill(null)
        .map(() => []);
...
function ReviewGrid() {
    ...
    return (
        ...
        >
            ...
            {column1.length > 0 && <ReviewColumn reviews={column1} msPerPixel={10} />}

            {column2.length > 0 && (
                <ReviewColumn reviews={column2} className={nonEmptyColumns === 2 ? "" : "hidden md:block"} msPerPixel={15} />
            )}

            {column3.length > 0 && <ReviewColumn reviews={column3} className="hidden lg:block" msPerPixel={10} />}
        </div>
    );
}
...
export function Reviews() {
    ...
    return (
        <section id="reviews" aria-labelledby="reviews-title" className="pt-12 pb-8 sm:pt-16 sm:pb-12 bg-pmred-500 overflow-hidden">
            <Container>
                <div className="flex flex-col items-center justify-between gap-8 md:flex-row md:items-center">
                    <div className="flex items-start gap-4">
                        <div className="hidden shrink-0 md:block">
                            <QuoteIcon className="h-20 w-20 text-white drop-shadow-[4px_4px_0px_rgba(36,67,141,1)]" />
                        </div>
                        <div className="flex flex-col items-center text-center md:items-start md:text-left">
                            <h1 id="reviews-title" className="font-dm-sans text-3xl font-bold text-white sm:text-4xl">
                            ...
                            <p className="mt-4 font-dm-sans text-xl font-medium text-white">We want to hear about it!</p>
                        </div>
                    </div>
